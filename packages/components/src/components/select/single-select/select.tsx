...
export class Choices implements IChoicesProps, IChoicesMethods {
  ...
  //custom props
  @Prop() myError: boolean = false;
  @Prop() myErrorMessage: string = "Error";
  @Prop() myLabel: string = "";
  @Prop() myDisabled: boolean = false;
  @Prop() myPlaceholderValue: string = "Placeholder";
  @Event() mySelect: EventEmitter<CustomEvent>;
  @Event() myInput: EventEmitter<CustomEvent>;
  @Prop() myOptions: any[] | string;
  @Prop() mySize: string = 'medium (40px)';
  ...
  @Method()
  async handleChange() {
    this.mySelect.emit(this.choice.getValue());
  ...
  public async setChoices(choices: any[] | string, value: string, label: string, replaceChoices?: boolean) {
    ...
    if (typeof choices === 'string') {
      ...
      console.error('Unexpected value for choices:', this.myOptions);
    }
  ...
  protected render(): any {
    ...
    // };
    const choicesWrapperClass = `choices__wrapper ${this.getSizeClass()}`;
    ...
    switch (this.type) {
        ...
        this.element =
          <div class={`select-container`}>
            ...
            ${this.myError ? 'error' : ""}`}
              ...
              onClick={this.myDisabled ? undefined : () => this.toggleDropdown()}
              onKeyDown={(event) => this.handleKeyDown(event)}
            >

              <select {...attributesSingle} data-trigger
                onChange={() => this.handleChange()}>
                {this.createSelectOptions(this.myOptions)}
              </select>


              <div class="choices__icon-wrapper-up">
                <icon
                  key='icon-up'
                  icon='chevronup-16'></icon>
              </div>
              <div class="choices__icon-wrapper-down">
      ...
      //             </div> : null
      //         }
      //         <div class={`${choicesWrapperClass} ${this.myDisabled ? 'disabled' : ""}`} onClick={this.myDisabled ? undefined : () => this.toggleDropdown()} >
      //           <select {...attributesMultiple} multiple onChange={() => this.handleChange()}>
      //             {this.createSelectOptions(this.myOptions)}
      //           </select>
      //           <div class="choices__icon-wrapper-up" onClick={this.myDisabled ? undefined : () => this.toggleDropdown()}>
      //             <icon
      //               icon='chevronup-16'></icon>
      //           </div>
      //           <div class="choices__icon-wrapper-down" onClick={this.myDisabled ? undefined : () => this.toggleDropdown()}>
      ...
      //     this.element =
      //       this.element =
      //       <div class={`select-container ${this.getSizeClass()}`}>
      //         {
      //           this.myLabel ?
      //             <div class="label-wrapper">
      //               <span>{this.myLabel}</span>
      //             </div> : null
      //         }
      //         <div class={`${choicesWrapperClass} ${this.myDisabled ? 'disabled' : ""} ${this.myError ? 'error' : ""}`}>
  ...
  toggleDropdown() {
    const div = this.root.querySelector('.choices__wrapper') as HTMLDivElement;
  ...
  closeDropdownMenu() {
    const myChoicesWrapper = this.root.querySelector('.choices__wrapper') as HTMLDivElement;
    this.handleClassList(myChoicesWrapper, 'remove', 'active');
  }
  ...
  handleOutsideClick(event: MouseEvent) {
    ...
    const myChoicesContainer = this.root.querySelector('.choices__wrapper') as HTMLDivElement;

    if (!path.includes(this.root)) {
      this.handleClassList(myChoicesContainer, 'remove', 'active');
    }
  }


  getSizeClass() {
    return `${this.mySize}` === "s"
      ? "small-select"
      : "medium-select";
  }
  ...
  private init() {
    ...
    if (element instanceof HTMLInputElement || element instanceof HTMLSelectElement) {
      ...
      if (this.type === 'single') {
            ...
            return {
                ...
                if (data.placeholder && !self.selectedOption?.value) {
                  ...
                  return template(`
                    <div class="choices__placeholder" data-item data-id="${data.id}" data-value="${data.value}" ${data.disabled ? 'aria-disabled="true"' : ''}>
                     ${data.label === undefined ? this.myPlaceholderValue : data.label}
                     ${removeButtonHTML}
                    </div>
                  `);
                } else {
              ...
              choice: ({ classNames }, data) => {
                ...
                ${data.disabled ? classNames.itemDisabled : classNames.itemSelectable} 
                  ...
                  ${data.selected || self.selectedOption?.value === data.value || self.getPreSelected(self)?.value === data.value ? '<icon icon="check16"></icon>' : ''}
                </div>
              `)
              },
            };
        ...
        //set select options
        this.setChoices(this.myOptions, "value", "label", true)
  ...
  private addEventListenersToHandleCustomFocusAndActiveState() {
    ...
    if (!div) {
      console.error('.choices__wrapper not found');
      return;
    }
  ...
  private addSearchEventListener(self, choiceElement: ChoicesJs) {
    choiceElement.passedElement.element.addEventListener(
      ...
      function (event: CustomEvent) {
        self.myInput.emit(event.detail.value)
      },
      false,
    );
  ...
  private getPreSelected(self) { //pass current context
    const optionValueBasedOnSelectedOption = Array.isArray(self.myOptions) ? self.myOptions.find(option => option.selected === true) : JSON.parse(self.myOptions).find(option => option.selected === true)
  ...
  private setPreSelected(newValue) {
    const resetPreviouslySelected = Array.isArray(this.myOptions) ? this.myOptions.map(obj => {
      return { ...obj, selected: false }
    }) : JSON.parse(this.myOptions).map(obj => {
      return { ...obj, selected: false }
    })
